...
def toFolds(fold, start, x, y=None):
    ...
    #     Input: x - pandas Series of the complete dataset
    #            y - pandas Series the complete set of labels
    ...
    if y:  # when labels are provided
        y_test = y[start:end]
        y_train = np.asarray(y.drop(range(start, end)))

        return np.asarray(x_test), y_test, np.asarray(x_train), y_train

    else:
        return np.asarray(x_test), np.asarray(x_train)
