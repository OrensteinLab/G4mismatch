...
def user_input():
    ...
    parser.add_argument('-u', '--use', help='Test with provided models, or train a new one.\n'
                        ...
                        choices=['train', 'test', 'cv'], type=str, required=False, default='test')
    ...
    parser.add_argument('-sc', '--scores', help='Target scores for fasta input in training mode with G4mismatchWG',
                        type=str, required=False)

    parser.add_argument('-om', '--other_model', help='Path to model different from those provided.',
                        type=str, required=False)

    parser.add_argument('-mc', '--model_feat', help='Input features for the PQ model',
...
def main():
    ...
    if args['g4mm_model'] != 'WG' or args['g4mm_model'] != 'PQ':
        ...
        if args['g4mm_model'] == 'g4mmWG' and args['use'] == 'cv':
            print('WG method is only used for train or test.')
            sys.exit()
